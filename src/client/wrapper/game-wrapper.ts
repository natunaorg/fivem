...
export default class Wrapper {
  ...
  ped = {
    ...
    teleportToMarker: async (ped: number = PlayerPedId()): Promise<boolean> => {
      ...
      for (let height = 1; height <= 1000; height++) {
        ...
        this.ped.teleportToCoordinates(ped, {
          x: coords[0],
          y: coords[1],
          z: height,
        });
    ...
    teleportToCoordinates: (
      ...
      SetPedCoordsKeepVehicle(
        ped,
        parseCoords(coords.x),
        parseCoords(coords.y),
        parseCoords(coords.z)
      );
  ...
  player = {
    ...
    setNoClip: async (speed = 2): Promise<boolean> => {
      ...
      const camHeading = this.client.utils.getCamDirection();
