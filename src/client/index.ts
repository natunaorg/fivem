...
export default class Client extends Events {
  ...
  constructor() {
    ...
    this.addSharedEventHandler(
      "natuna:client:setCommandDescription",
      this.setCommandDescription
    );
    this.addSharedEventHandler(
      `natuna:client:executeCommand`,
      (name: string, args: Array<any>, raw: string) => {
    ...
    this.addClientEventHandler(
      ...
      this._events.onClientResourceStart
    );
    this.addClientEventHandler(
      "onClientResourceStop",
      this._events.onClientResourceStop
    );
  }
    ...
    if (settings.nui) {
      if (settings.nui.debug) {
        this.addNUIEventHandler("natuna:client:nuiDebugSuccess", () => {
          this.utils.createFeedNotification(
            "NUI debug success, check your clipboard."
          );
        });
    ...
    if (settings.discordRPC) {
      ...
      const setRPC = () => {
        ...
        SetDiscordRichPresenceAssetText(
          parseRPCString(rpc.largeImage.hoverText)
        );

        SetDiscordRichPresenceAssetSmall(rpc.smallImage.assetName);
        SetDiscordRichPresenceAssetSmallText(
          parseRPCString(rpc.smallImage.hoverText)
        );
  ...
  _initClientPlugins = async (
    ...
    for (const plugin of plugins) {
      ...
      this.clientPlugins[plugin.name] = await import(
        `../../plugins/${plugin.name}/client/${plugin.file}`
      );
  ...
  _events = {
    ...
    onClientResourceStart: async (resourceName: string): Promise<void> => {
      if (GetCurrentResourceName() == resourceName) {
        ...
        this.triggerSharedCallbackEvent(
